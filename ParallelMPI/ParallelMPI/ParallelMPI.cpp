...
void rotateFace(Cube& cube, int face, int size) {
...
Cube solveCube(Cube& startCube, int numThreads) {
    ...
    // Initialize MPI (if not already done) and get rank
    int rank;
...
// Function to display the solution steps
void displaySolution(const std::vector<int>& solution) {
    std::cout << "Solution Steps:\n";
    for (int i = 0; i < solution.size(); ++i) {
        std::cout << "Step " << i + 1 << ": Rotate Face " << solution[i] << "\n";
    }
}
...
int main(int argc, char** argv) {
    ...
    int size; // Size of the Rubik's Cube (e.g., 2 for a 2x2x2 cube)
    ...
    while (true) {
        ...
        if (choice == 1) {
            ...
            // Call your cube-solving function here
            solveCube(cube, numThreads);

            if (isCubeSolved(cube)) {
                std::cout << "Rubik's Cube Solved!\n";
                displaySolution(rotationSteps);
            }
            else {
