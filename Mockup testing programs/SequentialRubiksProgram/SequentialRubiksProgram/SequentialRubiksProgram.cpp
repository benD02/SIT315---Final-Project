...
// Function to solve the Rubik's Cube using BFS
bool solveCube(Cube& startCube, std::vector<int>& solution) {
    std::queue<Cube> q;
    std::queue<std::vector<int>> moveQueue; // Queue to store the moves
    ...
    return false; // Cube cannot be solved
}
...
int main() {
    ...
    bool solved = solveCube(cube, solution);
